...
class Shopping_Agent_UCP_Auth extends Shopping_Agent_UCP_REST_Controller
{
    ...
    public static function validate_api_key_request($request, $required_permission = 'read')
    {
        ...
        if (empty($api_key)) {
            $api_key = $request->get_param('shopping_agent_ucp_api_key');
        }
        ...
        if (empty($api_key)) {
            ...
            return new WP_Error(
                'shopping_agent_ucp_auth_required',
        ...
        if (count($parts) !== 2) {
            return new WP_Error(
                'shopping_agent_ucp_invalid_key_format',
        ...
        if (!$key_data) {
            return new WP_Error(
                'shopping_agent_ucp_invalid_api_key',
        ...
        if ($key_level < $required_level) {
            return new WP_Error(
                'shopping_agent_ucp_insufficient_permissions',
                sprintf(
    ...
    public function delete_api_key($request)
    {
        ...
        if (!$result) {
            return $this->error_response(
                'shopping_agent_ucp_key_not_found',
