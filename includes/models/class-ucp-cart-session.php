...
class WC_UCP_Cart_Session
{
    ...
    {
        global $wpdb;
        $this->table_name = $wpdb->prefix . 'ucp_cart_sessions';
    }
    ...
    public function create($api_key_id = null)
    {
        ...
        $expiry_hours = (int) get_option('wc_ucp_cart_expiry_hours', 24);
        ...
        if ($result === false) {
            return new WP_Error(
                'ucp_cart_creation_failed',
                __('Failed to create cart.', 'ucp-shopping-agent'),
                array('status' => 500)
            );
        }
